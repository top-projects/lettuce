 ...
 * @author Mark Paluch
 */
public class RedisEndpoint implements RedisChannelWriter, Endpoint, HasQueuedCommands {

    private static final InternalLogger logger = InternalLoggerFactory.getInstance(RedisEndpoint.class);
    ...
    /**
     * Create a new {@link RedisEndpoint}.
     * 
     * @param clientOptions client options for this connection, must not be {@literal null}
     */
    public RedisEndpoint(ClientOptions clientOptions) {
    ...
    private void bufferCommand(RedisCommand<?, ?, ?> command) {
